...
uint8_t RX_flag2 =0;
uint8_t TC_flag3 =0;
USART_Config_t Global_USART_Config_s[3];
...
// Function to send the entire JSON string
void sendJSON(USART_REGISTERS_t *USARTx, char *jsonData) {
	  uint32_t i = 0;

	    // Send data until we hit the null terminator (assuming jsonData is null-terminated)
	    while (jsonData[i] != '\0') {  // Check for the null terminator

	        MCAL_USART_SendChar(USARTx, jsonData[i]);  // Send the low byte
	        i++;
	    }
}


// Function to receive the entire JSON string
void receiveJSON(USART_REGISTERS_t *USARTx, char *buffer, uint32_t bufferSize) {
    uint32_t index = 0;
    char receivedChar;

    // Clear the buffer first (optional)
    memset(buffer, 0, bufferSize);

    while (index < bufferSize - 1) {  // Leave space for the null terminator
        // Receive one character
        MCAL_USART_ReceiveChar(USARTx, &receivedChar);

        // If a newline or end of transmission is detected, break out of the loop (optional)
    ...
    // Null-terminate the string (in case it's not done already)
    buffer[index] = '\0';
}
